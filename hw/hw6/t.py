...
# s = input('Enter a nonempty string of lowercase letters: ').strip().lower()
# if s.isalnum():
#     print("all is lower")
...
#     if len(time_split) == 2:
#         hours, minutes = map(int, time_split[0].split(':') if ':' in time_split[0] else (time_split[0], 0))
...
if __name__ == "__main__":
        start_time = parse_time(input("Enter start time (HH:MM AM/PM): ").strip())
        ...
        end_time = parse_time(input("Enter end time (HH:MM AM/PM): ").strip())
        if end_time is None:
            print("Error: Invalid time format.")
        ...
        # Check for valid time format (HH:MM AM/PM)
